...
Git Hooks sind Scripts, die automatisch bei bestimmten Git-Aktionen ausgeführt werden. Sie ermöglichen es, Workflows zu automatisieren und Qualitätsstandards durchzusetzen.
...
## Hook manuell installieren

Falls du den Hook manuell installieren möchtest:

1. Kopiere die Datei `.githooks/pre-commit` nach `.git/hooks/pre-commit`
...
- Das Setup-Script sie nach `.git/hooks/` kopieren kann
...
### Beispiel: Commit-Message Länge prüfen
...
- Hooks sind Shell-Scripts und benötigen Git Bash
